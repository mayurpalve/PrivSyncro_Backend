...
exports.deleteConsent = async (req, res) => {
  try {
    ...
    const consent = await Consent.findOneAndDelete({ _id: id, userId: req.user.id });
    if (!consent) {
      return res.status(404).json({ message: "Consent not found" });
    }

    return res.status(200).json({ message: "Consent policy deleted" });
  } catch (error) {
    console.error("Delete consent error:", error.message);
    return res.status(500).json({ message: "Failed to delete consent" });
  }
};
